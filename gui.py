...
class ExpenseApp(tk.Tk):
    ...
    def _load(self, where="", params=()):
        ...
        for row in self.db.all(where, params):
            self.tree.insert("", "end", values=(
                row["id"], row["date"], row["category"], row["description"], f"{row['amount']:.2f}"
            ))

    def add(self):
        date = self.d.get().strip(); cat = self.c.get().strip() or "Other"
        desc = self.s.get().strip()
        ok, val_or_msg = utils.validate_amount(self.a.get())
        if not ok:
            messagebox.showerror("Error", val_or_msg); return
        amt = val_or_msg

        ok, msg = utils.validate_date(date)
        if not ok:
            messagebox.showerror("Error", msg); return

        # extra date strictness
        try:
            datetime.strptime(date, "%Y-%m-%d")
        except Exception:
            messagebox.showerror("Error", "Date format YYYY-MM-DD"); return

        self.db.add(date, cat, desc, amt)
        self._load()
        self.c.set(""); self.a.set(""); self.s.set("")
    ...
    def export(self):
        rows = self.db.all()
        path = utils.export_to_csv(rows, parent=self)
        if path:
            messagebox.showinfo("Exported", f"Saved {len(rows)} rows to {path.split(os.sep)[-1]}", parent=self)
